...
def addDatesToBoxscores( season, dir, day, enddate )
  while day < enddate
    ...
    #doc = Nokogiri::HTML( URI.open( "http://stats.nba.com/stats/scoreboardV2?DayOffset=0&LeagueID=00&gameDate=#{day.month}%2F#{day.day}%2F#{day.year}" ) )
...
def convertBookieNameToNBAAbbr( bookieName )
  abbr = ""
  if "BK" == bookieName
    abbr = "BKN"
  elsif "NY" == bookieName
...
def addNBAGameIDToBettingLineTable( season, type )
  tablename = season + "_" + type + "_" + "gamelogs"
  tablename = tablename.gsub(/[^0-9a-zA-Z_]/,'_').to_sym
  database = getDatabase(OPTIONS[:output])
  games = database[ tablename ].select_all.entries
  games = games.uniq{|g| g[:GAME_ID]}

  tablename = type + "_" + "bettinglines"
  tablename = tablename.gsub(/[^0-9a-zA-Z_]/,'_').to_sym

  games.each{|game|
    lines = database[:"#{tablename}"].where(:gameDate => game[:GAME_DATE], :final => "1").entries
    rows_updated = 0
    lines.each{|line|
      if line[:home] == game[:TEAM_ABBREVIATION] 
        rows_updated = database[:"#{tablename}"].select_all.where(:gameDate => game[:GAME_DATE], :home => game[:TEAM_ABBREVIATION]).update( {:nbaGameID => game[:GAME_ID]} )
        break
      elsif line[:away] == game[:TEAM_ABBREVIATION]
        rows_updated = database[:"#{tablename}"].select_all.where(:gameDate => game[:GAME_DATE], :away => game[:TEAM_ABBREVIATION]).update( {:nbaGameID => game[:GAME_ID]} )
        break
      end
    }
    if 0 == rows_updated
...
def getBettingLines( season, type, dir, day, enddate, bRecreate = false )
  tablename = type + "_" + "bettinglines"
  tablename = tablename.gsub(/[^0-9a-zA-Z_]/,'_').to_sym
  ...
  if (true == bRecreate) or !database.table_exists? tablename
    ...
    begin
      database.create_table tablename do
          ...
          column "home", String
          column "away", String
          column "point_spread", String
          column "point_spread_odds", String
          column "over_under", String
          column "over_under_odds", String
          column "final", String
          column "eventID", String
          column "gameDate", String
          column "nbaGameID", String
          column "point_spread_mean", String
          column "est_vegas_team_PTS", String
          column "est_vegas_opp_PTS", String
  ...
  while day < enddate
    #doc = Nokogiri::HTML( URI.open( "https://www.covers.com/sports/NBA/matchups?selectedday=#{day.year}-#{day.month}-#{day.day}" ) )
    doc = `curl 'https://www.covers.com/sports/NBA/matchups?selectedDate=#{day.year}-#{day.month}-#{day.day}' -H 'User-Agent: Mozilla/5.0 (Macintosh; Intel Mac OS X 10.14; rv:64.0) Gecko/20100101 Firefox/64.0' -H 'Accept: */*' -H 'Accept-Language: en-US,en;q=0.5' --compressed -H 'Referer: https://www.covers.com/Sports/NBA/Matchups?selectedDate=2010-11-25' -H 'X-Requested-With: XMLHttpRequest' -H 'DNT: 1' -H 'Connection: keep-alive' -H '#{OPTIONS[:cookie]}' -H 'TE: Trailers'`
    
    nok = Nokogiri::HTML( doc )

    nok.css("div.cmg_matchup_game_box").each{|game|
      row = Hash.new
      row["home"] = convertBookieNameToNBAAbbr( game.css("div.cmg_matchup_list_column_1 div.cmg_team_name").text.gsub("\r\n","").gsub(/\s|\d/,"") )
      row["away"] = convertBookieNameToNBAAbbr( game.css("div.cmg_matchup_list_column_3 div.cmg_team_name").text.gsub("\r\n","").gsub(/\s|\d/,"") )

      line_history_href = ""
      game.css("div.cmg_matchup_list_gamebox a").each{|a|
        if a.text.match(/Line History/i)
          line_history_href = a["href"]
        end
      }
      if "" == line_history_href
        #binding.pry
        p "No games found on #{day.year}-#{day.month}-#{day.day}"
        break
      end

      begin
        row["eventID"] = line_history_href.split("=").last
        row["gameDate"] = day.strftime( "%Y-%m-%d" )  #"#{day.year}-#{day.month}-#{day.day}"

        d2 = Nokogiri::HTML( URI.open( line_history_href ) )
      rescue StandardError => e
        binding.pry
        p 'hi'
      end
      ...
      rows = d2.css("table.covers-CoversOdds-lineHistoryTable tr");
      num_rows = rows.size
      bookname = nil
      rows.each_with_index{|tr,i|
        if 0 == i
          next
        end

        if tr.text.split.join(" ").match(/LINE.*TOTAL/)
          bookname = tr.css("th a img")[0]["alt"]
        elsif tr.text.split.join(" ").match(/Vegas.*Odds/i)
        else
          row["final"] = "0" 

          #check if it's the last row for the book (and thus final line)
          if i == (num_rows - 1) or rows[i+1].text.split.join(" ").match(/LINE.*TOTAL/)
            row["final"] = "1"
          end
